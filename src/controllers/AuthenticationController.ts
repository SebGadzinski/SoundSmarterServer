...
class AuthenticationController {
  ...
  public signUp(req: any, res: any) {
    if (req.user && !req.user.data.roles.includes('admin') && req.body.roles) {
      req.res.send(
        new Result({
  ...
  public emailConfirmStatus(req: any, res: any) {
    ...
    AuthenticationService.emailConfirmStatus(req?.user.data.email)
      .then((isConfirmed) => res.send(new Result({ data: isConfirmed, success: true })))
      .catch((err) =>
        res.send(new Result({
          message: err?.toString() ?? 'User not verified'
          , success: false
        }))
      );
  }
